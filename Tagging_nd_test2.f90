...
subroutine state_error(tag,tag_lo,tag_hi, &
  ...
  if (level .lt. max_phierr_lev) then
     do       k = lo(3), hi(3)
        do    j = lo(2), hi(2)
           do i = lo(1), hi(1)
           !Changed => to <= less than or equal for TEST2 refinement - mc2246
              if (state(i,j,k) .le. phierr(level)) then 
              !if state is greater than phierr for that level which you specify in probin file
				tag(i,j,k) = set
              endif
           enddo
        enddo
  ...
  if (level .lt. max_phigrad_lev) then
     do       k = lo(3), hi(3)
        do    j = lo(2)+1, hi(2)-1
           do i = lo(1)+1, hi(1)-1
			! Compare gradients in top bottom left right directions 
              ...
              ! Compute front and back gradients if dim=3
                 az = abs(state(i,j,k-1)-state(i,j,k))
                 az = max(az, abs(state(i,j,k)-state(i,j,k+1)))
              end if
              ! If the highest gradient is greater equal that phi gradient
              if (max(ax,ay,az) .le. phigrad(level)) then
                 tag(i,j,k) = set
              end if
            enddo
         enddo
